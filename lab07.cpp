...
bool isWinner(bool isPlayer1, int lastCol) {
  ...
  //get row of last move made
  int r = 0;
  while (board[lastCol][r] == EMP) {
	  r++;
  }
  ...
  //Using the position of the last move (board[c][r]) check all possible diagonals from that square and see if any of them are winners

  //diagonals going North-West to South-East
    if (board[lastCol][r] == currTile && board[lastCol+1][r+1] == currTile && board[lastCol+2][r+2] == currTile && board[lastCol+3][r+3] == currTile) {
    	return true;
    }
    else if (board[lastCol-1][r-1] == currTile && board[lastCol][r] == currTile && board[lastCol+1][r+1] == currTile && board[lastCol+2][r+2] == currTile) {
    	return true;
    }
    else if (board[lastCol-2][r-2] == currTile && board[lastCol-1][r-1] == currTile && board[lastCol][r] == currTile && board[lastCol+1][r+1] == currTile) {
        	return true;
    }
    else if (board[lastCol-3][r-3] == currTile && board[lastCol-2][r-2] == currTile && board[lastCol-1][r-1] == currTile && board[lastCol][r] == currTile) {
        	return true;
	}

  //diagonals going South-West to North-East
    else if (board[lastCol][r] == currTile && board[lastCol+1][r-1] == currTile && board[lastCol+2][r-2] == currTile && board[lastCol+3][r-3] == currTile) {
        	return true;
	}
	else if (board[lastCol-1][r+1] == currTile && board[lastCol][r] == currTile && board[lastCol+1][r-1] == currTile && board[lastCol+2][r-2] == currTile) {
		return true;
